...
defmodule LabGenerator do
  ...
  EEx.function_from_string(
    ...
    L'IA fornirà da 3 a 5 esempi pratici. Per sviluppare una memoria muscolare duratura, digita manualmente ogni esempio nella cella sottostante. Analizza le analogie fornite e leggi attentamente i commenti in linea: contengono la logica architetturale necessaria per padroneggiare questo argomento. Copia e incolla quanto segue nella tua chat IA, quindi invia.
    ...
    Questo è il prompt del mentore. Copialo e incollalo nel tuo assistente IA insieme al tuo codice e a una breve spiegazione della logica (tutto in un unico messaggio). Usalo per eseguire il debug degli errori, professionalizzare il tuo codice, richiedere una nuova sfida o ottenere una spiegazione più approfondita. Puoi anche porre al mentore domande specifiche o richiedere nuove sfide per testare le tue abilità. Questo simulatore di mentore d'élite fornisce feedback di alto livello per affinare il tuo intuito ingegneristico.
    ...
    ### 💻 Live Execution
    Usa la cella di esecuzione Elixir qui sotto per digitare manualmente ogni esempio e costruire una memoria muscolare duratura. 💡 Suggerimento Pro: puoi aggiungere altre celle di esecuzione cliccando sul pulsante + Elixir in fondo a qualsiasi sezione.
...
LabGenerator.run("elixir_otp_it.csv")
