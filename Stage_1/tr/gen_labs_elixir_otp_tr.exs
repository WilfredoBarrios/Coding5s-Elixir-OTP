...
defmodule LabGenerator do
  ...
  EEx.function_from_string(
    ...
    Yapay zeka 3 ila 5 pratik örnek sunacaktır. Kalıcı kas hafızası oluşturmak için her örneği aşağıdaki hücreye manuel olarak yazın. Sunulan analojileri analiz edin ve satır içi yorumları dikkatlice okuyun; bunlar bu konuya hakim olmanız için gereken mimari mantığı içerir. Aşağıdakini kopyalayıp yapay zeka sohbetinize yapıştırın ve gönderin.
    ...
    Bu mentör komutudur (prompt). Kodunuz ve kısa bir mantık açıklamasıyla birlikte yapay zeka asistanınıza kopyalayıp yapıştırın (hepsi tek bir mesajda olmalıdır). Hataları ayıklamak, kodunuzu profesyonelleştirmek, yeni bir meydan okuma istemek veya daha derin bir açıklama almak için kullanın. Ayrıca mentöre özel sorular sorabilir veya becerilerinizi test etmek için yeni görevler talep edebilirsiniz. Bu seçkin mentör simülatörü, mühendislik sezginizi keskinleştirmek için üst düzey geri bildirim sağlar.
    ...
    ### 💻 Live Execution
    Kalıcı kas hafızası oluşturmak için her örneği manuel olarak yazmak üzere aşağıdaki Elixir yürütme hücresini kullanın. 💡 İpucu: Herhangi bir bölümün altındaki + Elixir düğmesine tıklayarak daha fazla yürütme hücresi ekleyebilirsiniz.
...
LabGenerator.run("elixir_otp_tr.csv")
